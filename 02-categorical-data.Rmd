...
```{r bv-import, collapse=TRUE}
tv_data_bv <-
    read.table(system.file(
        "extdata", "tv.dat",   # without "doc" directory
        package = "vcdExtra")
        )

# or use the RStudio File -> Import Dataset -> From text (base) … menu.
...
```{r}
TV_xtabs <- xtabs(V5 ~ ., data = tv_data_bv)
dimnames(TV_xtabs) <-
...
# TV_min <- TV_xtabs[,,1:3,]     # keep only ABC, CBS, NBC
# TV_min <- TV_min[,,,3]       # keep only Persist -- now a 3 way table
# structable(TV_min)


TV2 <- as.data.frame(TV_xtabs, dim = c(5, 11, 5, 3))
```
